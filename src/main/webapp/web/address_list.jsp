        ...
        <div class="rs_content">
        	<form method="post" action="handle_add.do">
	            ...
	            <div data-toggle="distpicker" class="address_content">
					 ...
					 <select name="recvProvince"  id="recvProvince" onchange="getCities()">
					 <option value="0">----选择省----</option>
					 <c:forEach var="province" items="${provinces}">
					 <option value="${province.provinceCode}">${province.provinceName}</option>
					 </c:forEach>
					 </select>
					  城市：<select name="recvCity"  id="recvCity" onchange="getAreas()">
					  		<option value="0">----选择市----</option>
					  		
					  </select>
					  区/县：<select name="recvArea"  id="recvArea">
	            ...
	            <div class="address_tel">
	                ...
	                <input name="recvPhone" type="tel" id="receiverMobile" />
	                固定电话：<input name="recvTel" type="text"  id="receiverPhone"/>
	            </div>
	            <!--邮政编码-->
	            ...
	            <div class="address_name">
	                ...
	                <input type="text" id="addressName" name="recvTag"/>如：<span class="sp">家</span><span class="sp">公司</span><span class="sp">宿舍</span>
	            </div>
	            <!--保存收货人信息-->
	            <div >
		...
		$.ajax({
			...
			"success":function(obj){
				...
				for(var i=0;i<obj.date.length;i++){
					$("#recvCity").append("<option value="+obj.date[i].cityCode+">"+obj.date[i].cityName+"</option>");
				}
			}
		});
	}
	...
	if(cityCode!=0){
		$.ajax({
			...
			"success":function(obj){
				// obj.data
				for(var i=0;i<obj.date.length;i++){
					$("#recvArea").append("<option value="+obj.date[i].areaCode+">"+obj.date[i].areaName+"</option>");
				}
			}
		});
	}
